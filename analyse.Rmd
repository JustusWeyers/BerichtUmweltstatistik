...
plot.glmulti<-function(x, type="p", ...) 
{
	if ( type == "p") {
		...
		# draw
		par(oma=c(0,3,0,0))
		barplot(sort(imp),xlab="",xlim=c(0,1), ylab="",horiz=T,las=2, names.arg= c("Tiefe_zum_Ox.horizont", "Mächtigkeit_Ox.horizont", "Mächtigkeit_A", "Mächtigkeit_AL", "Geländehöhe"),main="Model-averaged importance of terms", ...)
		abline(v=0.8, col="red")
	} else	warning("plot: Invalid type argument for plotting glmulti objects.")
}

plot.glmulti(res, type = "s", cex.names = 0.7)
