...
```{r}
# Aufstellen eines GLM. Linkfunktion: gamme
...
```{r}
# Mögliche lineare Modelle nach Güte untersuchen und ordnen
resglm = glmulti(glm, level = 1, method = "h", confsetsize = 8, crit = "AIC")
# Das beste Modell übernehmen
bestglm = resglm@objects[[1]]
# Vorhersage und Fehler speichern
data = pred(name = "bestglm", model = bestglm, 
            predictors = predictors, data = data, 
            response = response)
```
...
# => p-value = 0.006113: Varianzen nicht gleich
oneway.test(data$errbestglm~data$StratumName)
# p-value = 4.773e-16 Ergebnis signifikant, Vorhersagen unterscheiden sich
```
...
data |> 
  ...
  pivot_longer(cols=c("errlinModel", "errbestlinModel", "errtreeModel", 
                      "errOOB", "errINB", "errglm", "errglm_gamma"), 
...
data |> 
  # Dataframe umgestalten. Spalte mit modelltyp hinzufügen.
  pivot_longer(cols=c("linModel", "bestlinModel", "glm", "treeModel", "OOB", "INB", "glm_gamma", "bestglm"), 
