...
# Funktion um Grafiken als svg-Datei zu exportieren
svgExport = function(plot, name){
  # Eventuell Dateiendung anhängen
  if (!endsWith(".svg", name)) name = paste0(name, ".svg")
  # Export
  svg(name, height=8,width=8)
  plot
  dev.off()
}
```
...
```{r, warning = FALSE, message=FALSE}
# Korrelationsmatrix
corplot = ggpairs(modeldata, columnLabels = str_wrap(c(predictsNames, responseName), width = 10), las = 2) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw()
# Export
svgExport(plot = corplot, name = file.path(imgpath, "corplot"))
