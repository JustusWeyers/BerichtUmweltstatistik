...
saveAsSVG = function(plot, name, h = 7) {
  plot |> ggsave(filename = file.path(imgpath, name), 
                 width = 15.5, height = h, units = "cm")
}
```
...
plot_linModelPrediction = ggplot(data |> drop_na(linModel), aes(x = Bodenfeuchte_Prozent, y = linModel)) +
  geom_point(aes(col = StratumName)) +
  scale_color_manual(values = wes_palette("Cavalcanti1", n = 3)) +
  labs(x = r"($y$)", y = r"($y_{linModel}$)", colour = "Stratum")+
  geom_abline(slope = 1) + annotate("text", x = 52, y = 25, label = "y = 1x") +
  theme_bw()
# Export als SVG
saveAsSVG(plot_linModelPrediction, "linModel/linModelPrediction.svg")

# Fehler ~ Stratum
plot_linModelError = ggplot(data |> drop_na(errlinModel), aes(x = StratumName, y = errlinModel)) +
...
saveAsSVG((plot_linModelPrediction|plot_linModelError), "linModel/linModel_page.svg")

# Barplot zur Cooks-Distance
{svg(file.path(imgpath, "linModel/linModel_cooksdistance.svg"), width = 15.5, height = 7)
...
saveAsSVG((p1+p2)/(p3+p4)/(p5+ plot_spacer()), "linModel/linModel_Regressionsgeraden.svg", h =10)

# Sind die Residuen normalverteilt?
normdist = ggplot(data.frame(x = linModel$residuals), aes(x = x)) + 
  geom_density() +
  labs(x = r"($y-y_{linModel}$)") +
  theme_classic()
saveAsSVG(normdist, "linModel/linModel_ResidualsDensity.svg")
shapiro.test(linModel$residuals)
```
...
svg(paste0(imgpath, "linModelAuto/AICs.svg"), width = 15.5, height = 7);plot(res);dev.off()
mean(res@crits)
...
plot_linModelAutoPrediction = ggplot(data |> drop_na(bestlinModel), aes(x = Bodenfeuchte_Prozent, y = bestlinModel)) +
  geom_point(aes(col = StratumName)) +
  scale_color_manual(values = wes_palette("Cavalcanti1", n = 3)) +
  labs(x = r"($y$)", y = r"($y_{linModelAuto}$)", colour = "Stratum")+
  geom_abline(slope = 1) + annotate("text", x = 52, y = 25, label = "y = 1x") +
  theme_bw()
plot_linModelAutoPrediction
# Export als SVG
saveAsSVG(plot_linModelAutoPrediction, "linModelAuto/linModelAutoPrediction.svg")

# Fehler ~ Stratum
plot_linModelAutoError = ggplot(data |> drop_na(errbestlinModel), aes(x = StratumName, y = errbestlinModel)) +
  geom_boxplot(aes(fill = StratumName)) +
  scale_fill_manual(values = wes_palette("Cavalcanti1", n = 3)) +
...
saveAsSVG((plot_linModelAutoPrediction|plot_linModelAutoError), "linModelAuto/linModelAuto_page.svg")
...
# Cooks Distance
{svg(file.path(imgpath, "linModelAuto/linModelAuto_cooksdistance.svg"), width = 15.5, height = 7)
...
#Cook´s Distance
{svg(file.path(imgpath, "glm/glm_cooksdistance.svg"), width = 15.5, height = 7)
...
```{r}
# Vorhersage GLM
plot_glmPrediction = ggplot(data |> drop_na(glm_gamma), aes(x = Bodenfeuchte_Prozent, y = glm_gamma)) +
  geom_point(aes(col = StratumName)) +
  scale_color_manual(values = wes_palette("Cavalcanti1", n = 3)) +
...
# Fehler ~ Stratum
plot_glmError = ggplot(data |> drop_na(errglm_gamma), aes(x = StratumName, y = errglm_gamma)) +
  geom_boxplot(aes(fill = StratumName)) +
  scale_fill_manual(values = wes_palette("Cavalcanti1", n = 3)) +
...
# Automatisierte Selektion eines generalisierten Modells
...
#Cook´s Distance
{svg(file.path(imgpath, "glmAuto/glmAuto_cooksdistance.svg"), width = 15.5, height = 7)
...
plot_glmAutoPrediction = ggplot(data |> drop_na(bestglm), aes(x = Bodenfeuchte_Prozent, y = bestglm)) +
  geom_point(aes(col = StratumName)) +
  scale_color_manual(values = wes_palette("Cavalcanti1", n = 3)) +
...
# Fehler ~ Stratum
plot_glmAutoError = ggplot(data |> drop_na(errbestglm), aes(x = StratumName, y = errbestglm)) +
  geom_boxplot(aes(fill = StratumName)) +
  scale_fill_manual(values = wes_palette("Cavalcanti1", n = 3)) +
...
saveAsSVG((plot_glmAutoPrediction|plot_glmAutoError), "glmAuto/glmAuto_page.svg")
```
...
# Stufen
treeLine = modeldata %>%
  mutate(pred = predict(treeModel, modeldata), StratumName = data$StratumName[as.numeric(rownames(modeldata))]) %>%
  ggplot(aes(x = Gelaendehoehe_m, y = Bodenfeuchte_Prozent)) +
  #geom_point(alpha = .2, size = 1, aes(col = StratumName)) +
  geom_point(aes(col = StratumName)) +
  scale_color_manual(values = wes_palette("Cavalcanti1", n = 3), name = "Stratum") +
...
treePred = data  |> 
  drop_na(treeModel) |>
  ggplot(aes(x = Bodenfeuchte_Prozent, y = treeModel, col = StratumName)) +
  geom_point() +
  scale_color_manual(values = wes_palette("Cavalcanti1", n = 3), name = "Stratum") +
  guides(color = guide_legend(nrow = 1)) +
  labs(x = r"($y$)", y = r"($y_{tree}$)") +
  geom_abline(slope = 1) + annotate("text", x = 60, y = 40, label = "y = 1x") +
  theme_bw() +
  theme(legend.position = "bottom")

plot_treeError = ggplot(data |> drop_na(errtreeModel), aes(x = StratumName, y = errtreeModel)) +
  geom_boxplot(aes(fill = StratumName)) +
  scale_fill_manual(values = wes_palette("Cavalcanti1", n = 3)) +
...
ptree = (treeLine|treePred|plot_treeError)
saveAsSVG(ptree, "CART/tree_page.svg")
```
...
{
par(mar = c(4, 8, 2, 0.5))
boxplot(data[,c("errlinModel", "errbestlinModel", "errglm_gamma", 
                "errbestglm","errtreeModel", "errINB")], las = 1, 
        horizontal = TRUE, xlab = "Fehlerquadrate")
}
```
