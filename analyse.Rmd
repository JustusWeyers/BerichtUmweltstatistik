...
```{r}
# Arbeitskopie von 'modeladata' erstellen
modeldata_transf = data.frame(modeldata)

# Box-Cox-Transformation der Response-Variable
bc = MASS::boxcox(linModel)
lambda = bc$x[which.max(bc$y)]
# modeldata$Bodenfeuchte_Prozent = BCTransform(y = modeldata$Bodenfeuchte_Prozent, lambda = lambda)

# Aufstellen eines GLM. Linkfunktion: log 
glm = glm(Bodenfeuchte_Prozent ~ ., data=modeldata, family=gaussian(link = "log"))
summary(glm)
plot(glm)
...
modeldata$Bodenfeuchte_Prozent = BCTransformInverse(modeldata$Bodenfeuchte_Prozent, lambda = lambda)
...
data |> 
  # Dataframe umgestalten. Spalte mit modelltyp hinzufügen.
  pivot_longer(cols=c("linModel", "bestlinModel", "glm", "treeModel", "OOB", "INB"), 
